...
jest.mock('@src/models/reading', () => {
  const mockFind = jest.fn().mockReturnValue({
    then: jest.fn().mockResolvedValue([{ customer_code: 'code1', measure_type: 'type1' }]), // Mock dos resultados
  });
...
describe('DatabaseService', () => {
  ...
  describe('getMeasuresByCustomerCode', () => {
    it('should fetch measures by customer code and type', async () => {
      const measures = await service.getMeasuresByCustomerCode('code1', 'type1');
      expect(measures).toEqual([{ customer_code: 'code1', measure_type: 'type1' }]);
    });
  ...
  describe('getReadingByCustomerAndType', () => {
    it('should fetch reading by customer code, type, and month', async () => {
      const reading = await service.getReadingByCustomerAndType('code1', 'type1', '2024-08');
